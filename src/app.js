...
app.post("/workouts", function(req, res) {
   ...
   const newPath = firebase.database().ref('gymData/previousWorkouts/' + newWorkoutId)
      ...
      workoutList.forEach(workout => {
        if (workout.name === childData.name){
          ...
          if (childData.previousWeights) {
              pathToUpdate = firebase.database().ref('gymData/exercises/' + childData.id + '/previousWeights/' + childData.previousWeights.length)
          } else {
              pathToUpdate = firebase.database().ref('gymData/exercises/' + childData.id + '/previousWeights/' +  0)
          }
          pathToUpdate.update({date: req.body.date, weight: workout.weight})
        }
      })
    })
  })
